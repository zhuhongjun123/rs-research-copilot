...
⚠️ **现状（v0.1.0）**：`build_chat()` 已被「审查报告摘要」使用；
   而 `build_embedder()` 及其下所有实现**尚未接入任何调用路径** ——
   它们是为「知识库检索层」预留的（该层未实现，见 README「尚未实现」）。
   保留这部分是因为其中的约束（向量不可混存、固定维度模型不能降维、
   限流需自行退避）是实测得来、且对后续实现仍然成立。
